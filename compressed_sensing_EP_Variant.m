...
%  recovery proplem using the Expectation Propergation Variant (Algorithm2)
%  in paper
%
...
%   Data=compressed_sensing_EP_Variant('M','b','SNR_dB','damp','sim_frames_str');
%
...
foldername = ['./sim_results/2021-01-17/Alg2-EPV/'];
...
for n_frame = 1:sim_frames
    ...
    for iter = 1:Nr_iter                                                                    %Alg2-Step 2
        ...
        beta      = (1-damp)*beta + damp*(y - mu_z_n)./(N0 + gamma_n);                      %Alg2-Step 7
        tau_t(:,1:N) = transpose(S'./(repmat(gamma_n,1,M) + N0-S'./transpose(tau_n_mtx)));  %Alg2-Step 8
        tau_0 = max(1e-8,sum(tau_t,2) - tau_t(:,N+1));                                      %Alg2-Step 9
        mu_x  = mean_x + (A'*beta)./tau_0;                                                  %Alg2-Step 10
        var_x = 1./tau_0;
        
        tmp_mu=(mu_x.*tau_x0+mu_x0.*var_x)./(var_x+tau_x0);                                 %Alg2-Step 11, calculate mean
        tmp_var = (tau_x0.*var_x)./(var_x+tau_x0);                                          %Alg2-Step 11, calculate variance
        mean_x  = tmp_mu./(1+...
        ...
        if ~isdeployed
            ...
            if  max(abs(est_x-mean_x)) < 1e-5                                               %Alg2-Step 12, if a station point is found
                break
            end
            est_x = mean_x;
        end
